...
{
	public partial class ChildFormKillJob : Form
	{
		public ChildFormKillJob()
		{
			InitializeComponent();
		...
		private void killJob_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("You failed to select a Protocol Type for this job." + System.Environment.NewLine + System.Environment.NewLine + "Make sure to select a Protocol Type in the drop down menu before hitting Kill Job", "Error");
			}
			else if (wfTextBox.Text == "WFXXXX")
			{
				MessageBox.Show("You failed to scan an assay barcode." + System.Environment.NewLine + System.Environment.NewLine + "Make sure to scan the Assay Barcode before hitting Kill Job", "Error");
			}
			else if (userComboBox.Text == null || userComboBox.Text == "")
			{
				MessageBox.Show("You failed to select a User Name for this job." + System.Environment.NewLine + System.Environment.NewLine + "Make sure to select a User in the drop down menu before hitting Kill Job", "Error");
			}
			else
			{
				string priority = "kill";
				Functions.AddJobToTempFolder(protocolComboBox.Text, userComboBox.Text, wfTextBox.Text, priority);
				this.Close();
			}
		}

		private void cancelKillJob_Click(object sender, EventArgs e)
		{
			this.Close();
		}
	}
